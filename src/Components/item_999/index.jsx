...
import "./style.css";
import item_999 from "../../data/item_999.json";
import { Link } from "react-router-dom";
import SearchItem from "../search";
import { FaInstagram } from 'react-icons/fa';

const Item_999 = () => {
  const [products, setProducts] = useState(item_999);
  ...
  const handleFilterClick = () => {
    if (!isFiltered) {
      const filtered = item_999.filter((item) =>
        item.title.toLowerCase().includes("item_999")
      );
      setProducts(filtered);
    } else {
      setProducts(item_999);
    }
  ...
  return (
    <>
      ...
      <div className="container">
        <div className="header">Under 999 </div>
